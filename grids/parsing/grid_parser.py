...
#     # Subjects seen in your samples; add more if needed
...
#     _NON_GRADE_FLAGS = {"Y", "PO", "CW", "CWK", "EXAM", "OR?", "REP", "R1", "R2", "R3", "R4", "R5"}
...
#         # Grade tokens are short (A, B+, C-, etc), not numbers, not flags like 'CW'
...
#         # Check if line starts a course (e.g. "COMP 1600" or "COMP" followed by "1600")
...
#         # Find where this course's data ends (next course or term footer)
#         j = start
#         while j < len(lines):
...
#         # credits = first float in window; quality_points = second float
...
#                     quality_points = val
#                 if floats_seen >= 2:
#                     # don't break; we still need the grade from later tokens
#                     pass
...
#     # --- Term GPA: last GPA-looking number before 'Term GPA' line ---
#     term_gpa: Optional[float] = None
#     for idx, ln in enumerate(lines):
#         if re.match(r"(?i)^Term\s+GPA", ln):
#             back = lines[max(0, idx - 8):idx]
#             for ln2 in reversed(back):
#                 m = _GPA_NUM_RE.search(ln2)
#                 if m:
#                     try:
...
def _extract_term_block_data(term_entry: Dict[str, str]) -> Dict[str, Any]:
    ...
    _SUBJ_RE = r"(COMP|INFO|MATH|FOUN|MGMT|ECON|ACCT|FSTF|BIOL|CHEM|PHYS|PSYC|SOCI|STAT|LAW|HIST|LING|ENGL|ESST|HOTL|AGEX|COCR|ENTR|FILM|GEND|HUEC|BIOC)"
